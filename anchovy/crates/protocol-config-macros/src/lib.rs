...
/// for back-compat with existing consumers. Non-scalar fields have only `_as_option`. Add
...
pub fn accessors_macro(input: TokenStream) -> TokenStream {
    ...
    let output = quote! {
        impl #struct_name {
            ...
            ///
            /// Non-scalar (e.g. list-typed) fields aren't represented here.
            pub fn attr_map(&self) -> std::collections::BTreeMap<String, Option<ProtocolConfigValue>> {
                vec![
...
/// accessor and a setter; scalar fields additionally populate
...
/// The extra tokens scalar fields contribute on top of the always-emitted accessor/setter
